...
<body>
    ...
    <div class="container">
        ...
        <form id="kc-otp-login-form" class="${properties.kcFormClass!}" action="${url.loginAction}" method="post" onsubmit="concatenateOTP(event)">
            ...
            <div id="kc-form-buttons">
                <div class="${properties.kcFormButtonsWrapperClass!} form-actions">
                    <input class="${properties.kcButtonClass!} cancel-btn ${properties.kcButtonDefaultClass!} ${properties.kcButtonLargeClass!}" name="cancel" type="submit" value="${msg("doCancel")}" />
                    <input class="${properties.kcButtonClass!} confirm-btn ${properties.kcButtonPrimaryClass!} ${properties.kcButtonLargeClass!}" name="login" type="submit" value="${msg("doLogIn")}" />
                </div>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        // Move focus to the previous input field and clear the current value on Backspace
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Add event listeners for backspace handling
            for (let i = 1; i <= 6; i++) {
                const currentInput = document.getElementById('input' + i);
                const prevInputId = i > 1 ? 'input' + (i - 1) : null;

                if (prevInputId) {
                    currentInput.addEventListener('keydown', (event) => handleBackspace(event, 'input' + i, prevInputId));
                }
            }
        });
        ...
        // Listen for keypress events on the last input
